...
import com.example.googleqrscanner.databinding.ActivityQrcodeGeneratorBinding
    ...
    private fun getUriFromBitmap(): Uri? {
        try {
            val file = File(
                ...
                "qr-code_${System.currentTimeMillis()}.png"
            )
            val outStream = FileOutputStream(file)
            qrBitmap?.compress(Bitmap.CompressFormat.PNG, 90, outStream)
            return FileProvider.getUriForFile(this, com.example.googleqrscanner.BuildConfig.APPLICATION_ID + ".provider", file)
